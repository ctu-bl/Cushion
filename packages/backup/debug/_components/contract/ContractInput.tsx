  ...
  return (
    <div className="flex flex-col gap-1.5 w-full">
      <div className="flex items-center ml-2">
        {paramType.name && (
          <span className="text-xs font-medium mr-2 leading-none text-base-content">{paramType.name}</span>
        )}
        <span className="block text-xs font-extralight leading-none text-base-content/70">{paramType.type}</span>
      </div>
      {renderInput()}
    </div>
  );
