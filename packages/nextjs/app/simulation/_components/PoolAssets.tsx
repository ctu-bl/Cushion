...
export const PoolAssets: React.FC<PoolAssetsProps> = ({ poolUSDCBalance, poolWETHBalance }) => (
  <SectionCard title="Mock Aave Pool Assets" className="h-full">
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <StatBox label="USDC in Pool">{poolUSDCBalance ? (Number(poolUSDCBalance) / 1e6).toFixed(2) : "0"} USDC</StatBox>
      <StatBox label="WETH in Pool">{poolWETHBalance ? (Number(poolWETHBalance) / 1e18).toFixed(4) : "0"} WETH</StatBox>
    </div>
  </SectionCard>
);
